...
// 			baseURL: 'https://gateway.ai.cloudflare.com/v1/f975428f0e5e4562c44ea2c44e1cba96/stock-predictions/openai',
// 		});
// 		try {
	...
	try {
		const stockData = await Promise.all(
			tickersArr.map(async (ticker) => {
				const url = `https://polygon-api-worker.rodion-yermakov-sd.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`;
...
async function fetchReport(data) {
	...
	try {
		const url = 'https://hidden-thunder-2548.rodion-yermakov-sd.workers.dev';
